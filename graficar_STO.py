...
def comparar_funcs(x : np.array, f, g : list, labels : list, linestyle : list, linewidth=2):
    ...
    for i in range(len(g)): # funciones aproximadas
        axs[0].plot(x, g[i], label=labels[i], linestyle=linestyle[i], linewidth=linewidth)
    ...
    for i in range(len(g)): # funciones aproximadas
        axs[1].plot(x, k*np.power(g[i],2), label=labels[i], linestyle=linestyle[i], linewidth=linewidth)
